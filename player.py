...
from utils import connect_to_redis, euclidean_distance, serialize
...
class KeyboardPlayerPyGame(Player):
    ...
    def show_target_images_default(self) -> None:
    ...
    def show_target_images(self) -> List:
        self.player_position = (0, 0)
        self.direction = 0
        targets = self.get_target_images()
        ...
        for target in targets:
            ...
            best_index = process_image_and_find_best_match(
                target, self.histograms, self.visual_dictionary
            )
        ...
        hor1 = cv2.hconcat(
            [self.images[best_indexes[0][0]], self.images[best_indexes[1][0]]]
        )
        hor2 = cv2.hconcat(
            [self.images[best_indexes[2][0]], self.images[best_indexes[3][0]]]
        )
        concat_img = cv2.vconcat([hor1, hor2])

        # Concatenate second best match images similarly as above
        hor1_second_best = cv2.hconcat(
            [self.images[best_indexes[0][1]], self.images[best_indexes[1][1]]]
        )
        hor2_second_best = cv2.hconcat(
            [self.images[best_indexes[2][1]], self.images[best_indexes[3][1]]]
        )
        ...
        hor1_third_best = cv2.hconcat(
            [self.images[best_indexes[0][2]], self.images[best_indexes[1][2]]]
        )
        hor2_third_best = cv2.hconcat(
            [self.images[best_indexes[2][2]], self.images[best_indexes[3][2]]]
        )
    ...
    def set_target_images(self, images) -> None:
        ...
        if self.target is not None:
            ...
            self.target_index = (
                int(
                    input(
                        f"Enter the row index (between 0 and {len(self.target) - 1}): "
                    )
                )
                - 1
            )
    ...
    def pre_navigation_bypass(self) -> None:
        if len(self.images) != 0:
            ...
            self.visual_dictionary = create_visual_dictionary(
                np.vstack(descriptors), num_clusters=100
            )
            print(f"Creating {len(self.images)} histograms")
            self.histograms = generate_feature_histograms(
    ...
    def find_targets(self) -> None:
        targets = self.get_target_images()
        for target in targets:
            best_indexes = process_image_and_find_best_match(
                target, self.histograms, self.visual_dictionary
            )
    ...
    def see(self, fpv: np.ndarray) -> None:
        ...
        def convert_opencv_img_to_pygame(opencv_image: np.ndarray) -> pygame.Surface:
            """
        ...
        self.poses.append(
            tuple([self.player_position[0], self.player_position[1], self.direction])
        )
    ...
    def update_map_on_keypress(self) -> None:
        self.direction %= 360
        move_x, move_y = 0, 0

        if self.key_hold_state[pygame.K_UP] or self.key_hold_state[pygame.K_DOWN]:
            move_amount = (
                MOVE_VALUE if self.key_hold_state[pygame.K_UP] else -MOVE_VALUE
            )
    ...
    def update_redis_data(self) -> None:
...
if __name__ == "__main__":
    ...
    arrow_process = subprocess.Popen([sys.executable, "arrow.py"])
